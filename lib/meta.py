...
exit = False
lock = threading.Lock()
result = open('result\\contentResult.txt', 'w') #lazy, easier than passing it through all functions, will correct later, I want this file open the entire time; garbage collection will close it for now, adding with for each  write will come later
	...
	'Takes metadata from page, sends it to televox.west equivalent'
        while not exit:
			...
			if url == oldBlogUrl:
				...
				lastSlash = url.rfind('/')
				if len(url[lastSlash:]) > 50:
					newUrl = newUrlFull[:newUrlFull.rfind('/', 0, len(newUrlFull) - 1) + 51]  #Assuming only last portion will exceed 50 chars
				else:	
					newUrl = newUrlFull
			...
			if descriptionLoc == None:
				...
				if descriptionLoc['content'][:16] != 'Learn more about' and descriptionLoc['content'][len(descriptionLoc['content']) - 4:] != '.com': #if description has 'Learn more about ... .com" but isn't bad -> False positive
