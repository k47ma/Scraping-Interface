...
def copyContent(session, lock, queue, siteName, newSiteName, blogUrl, libraryHtml):
	...
	while not exit:
		...
		if 'blog' in url:
			...
			iappsContainers = []
			for div in divs:
				if div.has_attr('class') and (div['class'] == 'iapps-container-container' or div['class'] == [u'iapps-container-container']):
        ...
        for script in scripts:
               	if 'nodeData' in script.text:
        	        rootIndex = script.text.find('value') #root will be the first result, next results will be folders, one for each on folder tree
                        ...
                        if collapseAnimationIndex < parentIndex:
                	        try:
					folderCreate = session.post(newUrl + 'TeleVox/WebServices/SharedContentService.asmx/CreateFolder', data={'parentId': rootID, 'folderName': 'Content'})
